...
/**
 * Contains reflection and metadata utility methods used by the module.
 *
 * <p><b>Refactoring: Extract Class</b> – serialization methods
 * ({@code serializeObject}, {@code serializeToJson}, {@code serializeCollectionItems},
 * {@code serializeMapItems}) have been moved to {@link AuditLogSerializer}.
 * This class now only contains reflection helpers and Hibernate metadata accessors.</p>
 */
public class AuditLogUtil {
	...
	// -----------------------------------------------------------------------
	// Deserialization helpers (read AuditLog data back from the DB)
	// -----------------------------------------------------------------------
	...
	public static Map<String, List> getChangesOfUpdatedItem(AuditLog auditLog) {
		if (auditLog.getAction() != UPDATED) {
			throw new APIException("Can't call this method for an AuditLog item with action "
			        + auditLog.getAction());
		}
	...
	/**
	 * Reads a {@link Blob} into a plain String.
	 */
	...
	/**
	 * Convenience method that finds a field with the specified name in the specified class.
	...
	/**
	 * Returns the actual (non-proxy) type of a persistent object.
	 */
	public static Class<?> getActualType(Object persistentObject) {
		...
		if (persistentObject instanceof HibernateProxy) {
			type = ((HibernateProxy) persistentObject).getHibernateLazyInitializer()
			        .getPersistentClass();
		}
		return type;
	}
	...
	/**
	 * Returns {@code true} when Hibernate has class metadata for the given class,
	 * meaning it is a mapped persistent entity.
	 */
	...
	/**
	 * Returns the Hibernate {@link ClassMetadata} for the given class, or {@code null}
	 * if the class is not a mapped entity.
	 */
	...
	/**
	 * Saves or updates a global property.
	 */
