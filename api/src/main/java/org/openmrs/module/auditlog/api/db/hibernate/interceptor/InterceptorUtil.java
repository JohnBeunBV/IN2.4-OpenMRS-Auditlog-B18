 ...
 *
 * <p>Refactoring: serialize* calls updated from AuditLogUtil to AuditLogSerializer
 * following the Extract Class refactoring of serialization responsibilities.</p>
 */
final class InterceptorUtil {
	...
	static String serializePersistentObject(Object object) {
		...
		if (cmd != null) {
			...
			propertyNameValueMap.put(cmd.getIdentifierPropertyName(),
			        cmd.getIdentifier(object, EntityMode.POJO));
				...
				if (value != null) {
					...
					if (cmd.getPropertyType(propertyName).isCollectionType()) {
						if (Collection.class.isAssignableFrom(value.getClass())) {
							// REFACTORING: was AuditLogUtil.serializeCollectionItems
							serializedValue = AuditLogSerializer.serializeCollectionItems((Collection) value);
						} else if (Map.class.isAssignableFrom(value.getClass())) {
							// REFACTORING: was AuditLogUtil.serializeMapItems
							serializedValue = AuditLogSerializer.serializeMapItems((Map) value);
						}
					} else {
						// REFACTORING: was AuditLogUtil.serializeObject
						serializedValue = AuditLogSerializer.serializeObject(value);
					}
		...
		// REFACTORING: was AuditLogUtil.serializeToJson
		return AuditLogSerializer.serializeToJson(propertyNameValueMap);
	}
