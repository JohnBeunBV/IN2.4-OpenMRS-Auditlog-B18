...
**Beschrijving:**  
Het endpoint `module/auditlog/exportAuditLog` is volledig anoniem toegankelijk. Elke gebruiker zonder login kan via een HTTP GET-request de volledige audittrail als CSV exporteren. Dit is een gecombineerde IDOR + unauthenticated access kwetsbaarheid. De `userId`-parameter wordt geaccepteerd maar nooit gebruikt als filter.
...
**Acceptatiecriteria:**
- [ ] SBOM-scan uitgevoerd op dependencies (door CI/CD groepsgenoot)
